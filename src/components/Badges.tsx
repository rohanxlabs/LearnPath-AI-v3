...
export function XPBadge({ amount, size = 'md' }: XPBadgeProps) {
  ...
  return (
    <div className={`inline-flex items-center font-semibold rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20 shadow-[0_0_12px_rgba(168,85,247,0.22)] ${sizeClasses[size]}`}>
...
export function StreakBadge({ days }: StreakBadgeProps) {
  return (
    <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 font-bold text-xs shadow-[0_0_10px_rgba(245,158,11,0.12)]">
...
export function TierBadge({ isPro, onClick }: TierBadgeProps) {
  return (
    <button
      onClick={onClick}
      disabled={isPro}
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-semibold text-xs border transition-all duration-300 ${
        isPro
